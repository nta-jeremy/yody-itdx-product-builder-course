...
6. [Khái niệm 4 — Temperature: núm vặn độ ngẫu nhiên](#6-khái-niệm-4--temperature-núm-vặn-độ-ngẫu-nhiên)
7. [Khái niệm 5 — Embeddings: tìm theo nghĩa](#7-khái-niệm-5--embeddings-tìm-theo-nghĩa)
8. [Tổng hợp — Mọi giới hạn của AI là infrastructure constraint](#8-tổng-hợp--mọi-giới-hạn-của-ai-là-infrastructure-constraint)
...
Không. Đây không phải lỗi. Đây là **một giới hạn có cấu trúc** gọi là *context window* — và nếu bạn không biết nó tồn tại, bạn sẽ thiết kế tính năng sản phẩm theo cách bảo đảm thất bại từ đầu.

> 🖼️ **[CẦN HÌNH]** Minh họa hai màn hình cạnh nhau: (1) người dùng thông thường hỏi AI "làm sao để câu trả lời hay hơn?", (2) người build sản phẩm hỏi "giới hạn nào của mô hình sẽ ảnh hưởng đến quyết định sản phẩm của tôi?". Phong cách: flat editorial illustration, màu #2a2b86 và #fcaf16 trên nền trắng ngà, Montserrat, 16:9.
...
- **Người build sản phẩm bằng AI** hỏi: *"Giới hạn nào của mô hình sẽ ảnh hưởng đến quyết định sản phẩm của tôi — và tôi thiết kế quanh nó như thế nào?"*

Cả hai đều cần AI. Nhưng người build sản phẩm cần thêm một thứ: **hiểu AI như một hệ thống hạ tầng có giới hạn**, không phải như một phép màu vô hạn.
...
Cuối buổi, bạn làm được ba việc sau:

**Việc 1:** Giải thích được 5 khái niệm nền tảng của AI (LLM, token, context window, temperature, embeddings) và nêu được ít nhất 1 hệ quả cụ thể đối với sản phẩm cho mỗi khái niệm.

**Việc 2:** Phân tích được một quyết định product cụ thể có liên quan đến giới hạn context window — bao gồm: điểm đụng giới hạn ở đâu, có những phương án xử lý nào, và trade-off (ai phải trả giá gì).

**Việc 3:** Phân biệt được AI làm được và không làm được gì — và nêu được ít nhất 1 use case AI phù hợp với công việc product của bạn.

> **Lưu ý quan trọng:** Mục tiêu không phải là "thuộc định nghĩa". Nếu bạn đọc xong và chỉ nhớ được định nghĩa mà không nói được *"điều này ảnh hưởng đến thiết kế sản phẩm thế nào"* — thì bạn chưa đạt mục tiêu buổi học.

---

## 3. Khái niệm 1 — LLM thực sự làm gì: dự đoán token

> 🖼️ **[CẦN HÌNH]** Minh họa câu "Yody là thương hiệu thời ___" với các từ có thể điền vào cùng thanh xác suất (ví dụ: "trang" 80%, "tiết" 5%, "đại" 3%...). Bên dưới caption: "Mô hình đoán token — không tra cứu sự thật". Phong cách: flat editorial illustration, màu #2a2b86 và #fcaf16.
...
Cơ chế cốt lõi của LLM cực kỳ đơn giản về mặt nguyên lý: **dự đoán token (mảnh chữ) tiếp theo theo xác suất**.
...
Khi LLM trả lời một câu hỏi, nó **không mở một cơ sở dữ liệu chứa sự thật rồi tra ra câu trả lời đúng**. Nó chỉ đang dự đoán chuỗi từ nghe **hợp lý nhất** dựa trên xác suất.
...
1. **Không dùng LLM như nguồn dữ liệu thật.** Mọi con số, tên sản phẩm, ngày tháng, thông tin kỹ thuật quan trọng từ LLM đều cần được kiểm chứng từ nguồn có thẩm quyền.

2. **Thiết kế luồng kiểm chứng trong sản phẩm.** Nếu bạn dùng AI để tóm tắt dữ liệu nội bộ, hãy để dữ liệu nội bộ đó *đi vào trong prompt* (thay vì nhờ AI "nhớ" từ trước) — và có cơ chế xác nhận output quan trọng.
...
> **Câu tóm gọn để nhớ:** *"LLM đoán ngôn ngữ, không tra cứu sự thật — output trơn tru không đồng nghĩa với output đúng."*
...
- Một phần của từ: "khoác" có thể được tách thành "kho" + "ác" tùy mô hình
- Dấu câu hoặc ký tự đặc biệt: ".", ",", "!"

Quy tắc gần đúng (không chính xác tuyệt đối): **1 token ≈ 4 ký tự tiếng Anh**, hoặc khoảng **3/4 từ tiếng Anh thông thường**.

Ví dụ: câu *"The quick brown fox"* có 4 từ ≈ 4 token.

### Tại sao token quan trọng với sản phẩm?

Có ba lý do trực tiếp chạm đến quyết định thiết kế sản phẩm:
...
Mỗi mô hình có **giới hạn tối đa số token** có thể xử lý trong một lượt. Đây chính là *context window* — sẽ được giải thích chi tiết ở mục tiếp theo.

### Tiếng Việt tốn token hơn tiếng Anh

Đây là một điểm thực tế quan trọng khi làm sản phẩm cho thị trường Việt Nam.

Hầu hết các LLM được huấn luyện chủ yếu bằng tiếng Anh. Cách chúng tách từ (tokenizer) được tối ưu cho tiếng Anh. Khi xử lý tiếng Việt — đặc biệt là tiếng Việt có dấu — mô hình thường phải tách mỗi từ thành nhiều token hơn so với từ tương đương trong tiếng Anh.
...
> 🖼️ **[CẦN HÌNH]** Minh họa "cửa sổ" có kích thước cố định chứa các token. Khi nhồi quá nhiều, phần token ở đầu bị đẩy ra ngoài và rơi đi. Caption: "Nhồi nhiều ≠ tốt hơn". Phong cách: flat editorial illustration, màu #2a2b86 và #fcaf16.
...
Context window (cửa sổ ngữ cảnh) là **bộ nhớ làm việc của mô hình trong một lượt xử lý**. Nó định nghĩa số token tối đa mà mô hình có thể "nhìn thấy" và xử lý trong một lần gọi.

Để hình dung, hãy tưởng tượng bạn đang làm việc trên một tờ giấy có kích thước cố định. Tờ giấy đó chứa được N từ. Khi bạn viết thêm vào cuối, phần đầu bị đẩy ra ngoài tầm nhìn. Mô hình AI chỉ "thấy" những gì đang nằm trong tờ giấy đó — phần bị đẩy ra thì không còn tồn tại với nó nữa.
...
**Câu thần chú:** *"Nhồi nhiều ≠ tốt hơn."*

### Ba chiến lược thiết kế quanh context window

> 🖼️ **[CẦN HÌNH]** Ba ô cạnh nhau mô tả ba chiến lược: (1) Chunking — chia dữ liệu thành batch nhỏ, xử lý từng batch; (2) Tóm tắt phân tầng — tóm tắt từng phần rồi tổng hợp; (3) RAG — chỉ kéo phần liên quan vào. Mỗi ô có tag nhỏ ghi trade-off. Phong cách: flat editorial illustration, màu #2a2b86 và #fcaf16.
...
**Trade-off — ai trả giá:**

| Lợi ích | Chi phí |
|---------|---------|
| Bao phủ toàn bộ dữ liệu | Tăng số lần gọi API → tăng chi phí tổng |
| Không bỏ sót | Tăng độ trễ tổng (phải chờ nhiều lần) |
| Dễ triển khai kỹ thuật | Cần xử lý logic gộp kết quả |
...
| Lợi ích | Chi phí |
|---------|---------|
| Tiết kiệm context window đáng kể | Mất chi tiết khi tóm tắt |
| Phù hợp với dữ liệu rất dài | Rủi ro bỏ sót sắc thái quan trọng |
| Giảm chi phí token | Cần bước tiền xử lý (tóm tắt) trước |
...
| Lợi ích | Chi phí |
|---------|---------|
| Chính xác cao vì có đúng ngữ cảnh cần thiết | Phức tạp về kỹ thuật hơn nhiều |
| Tiết kiệm token (chỉ kéo phần liên quan) | Cần hạ tầng vector database |
| Scale được với dữ liệu rất lớn | Chi phí setup và vận hành cao hơn |
...
|----------|-------------------|
| Dữ liệu vừa phải, cần xử lý hết | Chunking |
| Dữ liệu rất dài, không cần từng chi tiết | Tóm tắt phân tầng |
| Dữ liệu lớn, cần độ chính xác cao, có đội kỹ thuật | RAG |
| Dùng nội bộ, yêu cầu thấp, nguồn lực hạn chế | Chunking + Tóm tắt phân tầng |
...
## 6. Khái niệm 4 — Temperature: núm vặn độ ngẫu nhiên

> 🖼️ **[CẦN HÌNH]** Thanh trượt từ 0 đến 1. Phía trái (gần 0): biểu tượng ổn định, lặp lại, ví dụ "viết spec kỹ thuật". Phía phải (gần 1): biểu tượng đa dạng, sáng tạo, ví dụ "đặt tên sản phẩm". Chú thích: "Không phải núm độ thông minh — là núm độ ngẫu nhiên". Phong cách: flat editorial illustration, màu #2a2b86 và #fcaf16.
...
Đây là điểm hay bị hiểu nhầm: **temperature không làm cho AI thông minh hơn hay ngu hơn**. Nó chỉ điều chỉnh sự biến thiên trong output.
...
- **Temperature cao** = nhân viên đó trong trạng thái brainstorm thoải mái, đưa ra nhiều ý tưởng khác nhau, đôi khi táo bạo, đôi khi lạ.
...
> 🖼️ **[CẦN HÌNH]** Minh họa văn bản chuyển thành vector (dãy số). Các câu gần nghĩa ("vải nóng", "mặc bí mồ hôi", "đổ mồ hôi lưng") nằm gần nhau trong không gian 2D. Các câu không liên quan ("giao hàng nhanh", "khóa kéo bị kẹt") nằm xa nhau. Phong cách: flat editorial illustration, màu #2a2b86 và #fcaf16.
...
Embeddings là nền tảng của nhiều tính năng AI phổ biến. Ở mức L1 (buổi học này), bạn chỉ cần hiểu ý niệm — không cần biết cách code.
...
## 8. Tổng hợp — Mọi giới hạn của AI là infrastructure constraint
...
**Tất cả đều là giới hạn có cấu trúc.** Không phải lỗi, không phải sự cố kỹ thuật nhất thời — mà là đặc tính vốn có của hệ thống.

> 🖼️ **[CẦN HÌNH]** Minh họa AI đứng cạnh DATABASE và API, cả ba đều có đồng hồ đo giới hạn (gauge). Caption: "AI cũng là infrastructure — có giới hạn như mọi hệ thống khác". Phong cách: flat editorial illustration, màu #2a2b86 và #fcaf16.
...
Và khi thiết kế sản phẩm, bạn **thiết kế quanh** những giới hạn đó: cache, pagination, retry logic, async processing...

**AI cũng như vậy.** Token là giới hạn về chi phí và tốc độ. Context window là giới hạn về bộ nhớ làm việc. Hallucination là giới hạn về độ tin cậy. Temperature cần được chọn đúng để không có sai sót.

### Tư duy người build sản phẩm

Đây là điểm khác biệt lớn nhất giữa người dùng AI và người build sản phẩm bằng AI:

**Người dùng AI nghĩ:**
> *"AI này hay đấy, mình sẽ dùng nó để làm X."*

**Người build sản phẩm bằng AI nghĩ:**
...
**Câu 3:** **Trade-off là gì — ai trả giá?** Với mỗi phương án bạn đề xuất, nêu rõ: (a) điều gì bị đánh đổi, (b) ai là người chịu hậu quả (nhân viên CSKH? khách hàng? đội kỹ thuật? doanh nghiệp?).
...
Viết **1 use case AI** cho công việc product của bạn. Không cần hoàn hảo — đây là "phôi" để phát triển ở buổi sau (I1.2).
...
|-----------|-------------|-------------------------------|
| **LLM dự đoán token** | Đoán ngôn ngữ, không tra sự thật | Kiểm chứng mọi thông tin quan trọng từ AI |
...
*YODY Product Builder Program — Tài liệu học I1.1 · v1.0 · 25/06/2026 · Dành cho người học*
